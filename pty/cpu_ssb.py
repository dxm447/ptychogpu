...
def subpixel_pad4D(data4D_flat,final_size):
    ...
    for ii in range(data4D_flat.shape[0]):
        ...
        padded_4D[ii,:,:] = moved_cbed
    return padded_4D

def get_G_matrix(data4D):
    data4D = np.transpose(data4D,(2,3,0,1)) #real in 2,3 
    data4D = np.fft.fftshift(np.fft.fft2(data4D,axes=(-2,-1)),axes=(-2,-1)) #now real is Q' which is 2,3
    data4D = np.transpose(data4D,(2,3,0,1)) #move original Q to 2,3 and Q' to 0,1
    data4D = np.fft.ifftshift(np.fft.ifft2(data4D,axes=(-2,-1)),axes=(-2,-1)) #now 0,1 is Q' and 2,3  is R' 
    data4D = np.transpose(data4D,(2,3,0,1)) #0,1 is R' and 2,3 is Q'
    return data4D
